...
def checkout(request):
    try:
        ...
        loyalty_point = profile.loyalty_point
        print("***************\n",loyalty_point,"\n***************\n")
        if loyalty_point != 0 and loyalty_point % 4 == 0:
            price_paid = 0.0
...
@api_view(['GET'])
@permission_classes((permissions.AllowAny,))
def transaction(request, customer_id):
    try:
        trans = Transaction.objects.filter(customer_id__customer_id=customer_id)
        serializer = TransactionSerializer(trans, many=True)
        return Response(serializer.data)
    except:
        return Response("Error: No transactions found for the customer.", status=status.HTTP_400_BAD_REQUEST)
